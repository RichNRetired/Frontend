...
export default function AddressesPage() {
  ...
  const confirmDelete = async () => {
    ...
    setLocalAddresses((prev) => prev.filter((a) => a.id !== pendingDeleteId));
    setDeletingId(pendingDeleteId);
    try {
      await deleteAddress(pendingDeleteId).unwrap();
      refetch();
      setFeedbackTone("success");
      setFeedbackMessage("Address removed successfully.");
      setPendingDeleteId(null);
    } catch {
      setLocalAddresses(previous);
      setFeedbackTone("error");
      setFeedbackMessage("Failed to remove address.");
    } finally {
      setDeletingId(null);
    }
  };
        ...
        <div className="mb-16 border-b border-black pb-8">
          <div className="flex items-start justify-between">
            <div>
              ...
              {feedbackMessage && (
                <div className={`mt-4 flex max-w-md items-start gap-3 rounded-lg border px-4 py-3 text-sm ${feedbackTone === "error" ? "border-red-200 bg-red-50 text-red-700" : "border-green-200 bg-green-50 text-green-700"}`}>
                  {feedbackTone === "error" ? <AlertCircle size={18} className="mt-0.5 shrink-0" /> : <CheckCircle2 size={18} className="mt-0.5 shrink-0" />}
