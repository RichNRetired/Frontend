...
import { useAddAddressMutation, useUpdateAddressMutation } from "@/features/user/userApi";
...
export const AddressFormModal: React.FC<AddressFormModalProps> = ({
  ...
  useEffect(() => {
    if (mode === "edit" && editingAddress) {
      ...
      const matchedLocation = serviceableLocations.find(
        (location) => location.pincode === String(editingAddress.postalCode || ""),
      );
      setSelectedLocationId(matchedLocation ? String(matchedLocation.id) : "");
    } else {
      resetForm();
    }
  }, [mode, editingAddress, serviceableLocations, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.data?.message || "Failed to save address. Please try again.");
    }
  };
